...
<section id="pricing" class="relative bg-background py-24 sm:py-32">
	<div class="mx-auto max-w-7xl px-6 lg:px-8">
		...
		<div class="mt-12 grid gap-6 lg:grid-cols-3">
			{#each plans as plan}
				<div
					class={`flex flex-col gap-6 rounded-2xl border p-6 shadow-lg ${plan.bestValue ? "border-primary bg-primary/8 ring-1 ring-primary/40 lg:-translate-y-2" : "border-border/70 bg-card/40"}`}
				>
					<div class="flex items-start justify-between gap-3">
						<div>
							<p class="font-heading text-2xl font-bold text-foreground">
								{plan.name}
							</p>
					...
					<div class="flex items-end gap-2">
						<span class="font-heading text-5xl font-extrabold text-foreground"
							>{plan.price}</span
						>
						<span class="pb-1 text-muted-foreground">{plan.period}</span>
					</div>
					...
					<ul class="space-y-3">
						{#each plan.features as feature}
							...
							>
								<CircleCheckIcon class="mt-0.5 size-4 shrink-0 text-primary" />
								<span>{feature}</span>
							</li>
						{/each}
					</ul>

					<Button href="/auth/login" class="w-full" size="lg">
						Start Plan
					</Button>
				</div>
			{/each}
